"""
app.py — Bucle principal de la experiencia interactiva.

Novedades respecto a la versión anterior:
  - Comprueba `state_manager.exit_requested` cada frame; si es True
    rompe el bucle limpiamente para que el watchdog pueda leer state.json
    y lanzar la demo seleccionada.
"""
...
def _rescale_hands(hands_info: List[HandInfo], from_size, to_size) -> List[HandInfo]:
    """Reescala todos los puntos de cada HandInfo de `from_size` a `to_size`."""
    if not hands_info:
        return []
    fw, fh = from_size
    tw, th = to_size
...
class InteractiveApp:
    def __init__(self):
        ...
        # Fondo pre-alocado (evita 6 MB por frame)
        ...
        self.hand_tracker = HandTracker(
            ...
            min_presence_confidence=getattr(settings, "MIN_PRESENCE_CONFIDENCE", None),
        )
    ...
    def _init_camera(self):
        ...
        if not cap.isOpened():
            ...
            raise RuntimeError(
                ...
                "Verifique que esté conectada y libre."
            )
    ...
    def _init_window(self):
        ...
        if settings.FULLSCREEN:
            ...
            cv2.resizeWindow(settings.WINDOW_NAME,
                             settings.WINDOW_WIDTH, settings.WINDOW_HEIGHT)

    def _toggle_fullscreen(self):
        self._fullscreen = not self._fullscreen
        if self._fullscreen:
            cv2.setWindowProperty(settings.WINDOW_NAME,
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Reducir resolución para MediaPipe
                pw, ph = settings.PROCESSING_WIDTH, settings.PROCESSING_HEIGHT
                small = (cv2.resize(frame, (pw, ph), interpolation=cv2.INTER_LINEAR)
                         if (cam_w, cam_h) != (pw, ph) else frame)
                ...
                # ── Salida por demo seleccionada ────────────────────────────
                # state.json ya fue escrito por DemoMenuState._launch_demo()
                if self.state_manager.exit_requested:
                    print("[app] Demo seleccionada. Cerrando menú para el watchdog.")
                    break
                ...
                if settings.SHOW_TIMING_PROFILE:
                    ...
                    if time.time() - prof_t0 >= 2.0:
                        avg_r = 1000 * t_read  / prof_frames
                        avg_p = 1000 * t_proc  / prof_frames
                        avg_d = 1000 * t_render / prof_frames
                        avg_t = avg_r + avg_p + avg_d
                        print(
                            f"[profile] cam+resize={avg_r:.1f}ms | "
                            f"mediapipe={avg_p:.1f}ms | "
                            f"render={avg_d:.1f}ms | "
                            f"total={avg_t:.1f}ms ({1000/avg_t:.1f} FPS)"
                        )
                        t_read = t_proc = t_render = 0.0
                        prof_frames = 0
                        prof_t0 = time.time()
    ...
    def _draw_fps(self, canvas):
        ...
        cv2.putText(canvas, f"FPS: {self._fps_current:.1f}",
    ...
    def _draw_extended_fingers_debug(self, canvas, hands_info: List[HandInfo]):
        ...
        cv2.putText(canvas, text, (20, canvas.shape[0] - 30),
